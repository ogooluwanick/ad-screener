...
import { Camera, Mail, Calendar, MapPin, Award, AlertTriangle, Loader2, CheckCircle, XCircle, Briefcase, Link as LinkIcon } from "lucide-react"
...
interface UnifiedProfileData extends UserProfileData {
  image?: string; // For display (can be base64 preview or server URL)
  phone: string;
  location: string;
  bio: string;
  joinDate: string;
...
export default function UnifiedProfilePage() {
  ...
  useEffect(() => {
    if (fetchedProfileData) {
      setProfileData(prev => ({
        ...initialUnifiedProfileData, // Start with full initial state to reset mocks if role changed
        ...fetchedProfileData,     // Overlay with fetched common data
        // Ensure role-specific fields from fetched data are applied, or retain mocks if not present in fetched
        role: fetchedProfileData.role || initialUnifiedProfileData.role,
        image: fetchedProfileData.profileImageUrl || initialUnifiedProfileData.image, // Use profileImageUrl from DB
        ...
        // Reviewer specific fields - if they are not part of UserProfileData, they'll use initialUnifiedProfileData values
  ...
  const handleSave = () => {
    let payload: UpdateUserProfilePayload = {
      ...
      bio: profileData.bio,
      profileImageUrl: newlyUploadedImageUrl || profileData.image, // Use new URL if uploaded, else existing
    };
    ...
    if (profileData.role === "submitter") {
      ...
      // Assuming department is part of UpdateUserProfilePayload for reviewers
      // payload = { ...payload, department: profileData.department };
    }
  ...
  const handleCancelEdit = () => {
    setIsEditing(false);
    setNewlyUploadedImageUrl(null); // Clear any pending image upload
    if (fetchedProfileData) {
      // Revert to last fetched state
       setProfileData(prev => ({
        ...initialUnifiedProfileData,
        ...fetchedProfileData,
        image: fetchedProfileData.profileImageUrl || initialUnifiedProfileData.image,
        role: fetchedProfileData.role || initialUnifiedProfileData.role,
        // ensure all fields are correctly reverted
        department: (fetchedProfileData as UnifiedProfileData).department || initialUnifiedProfileData.department,
        expertise: (fetchedProfileData as UnifiedProfileData).expertise || initialUnifiedProfileData.expertise,
        reviewerLevel: (fetchedProfileData as UnifiedProfileData).reviewerLevel || initialUnifiedProfileData.reviewerLevel,
      }));
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      setProfileData((prev) => ({ ...prev, image: reader.result as string })); // Show local preview
    };
    ...
    try {
      ...
      setNewlyUploadedImageUrl(result.imageUrl); // This URL will be saved
      setProfileData((prev) => ({ ...prev, image: result.imageUrl })); // Update preview to server URL
          ...
          {!isEditing ? (
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={handleSave} disabled={isUpdatingProfile || isUploadingImage} className="bg-blue-600 hover:bg-blue-700">
                {isUpdatingProfile ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving Profile...</> : 
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent className="space-y-6">
            ...
            {userRole === 'reviewer' && (<div className="space-y-2">
                <Label>Areas of Expertise (Not Saved with main profile)</Label>
                <div className="flex flex-wrap gap-2">{(profileData.expertise || []).map((skill, index) => (<Badge key={index} variant="secondary">{skill}</Badge>))}</div>
                {isEditing && <Input placeholder="Add expertise (comma-separated)" defaultValue={(profileData.expertise || []).join(", ")} onChange={(e) => handleInputChange("expertise", e.target.value.split(",").map(s => s.trim()))} disabled={isUploadingImage} />}
            </div>)}
          </CardContent>
        </Card>

        {userRole === 'reviewer' && (<>
            <Card className="md:col-span-3"><CardHeader><CardTitle>Review Performance</CardTitle><CardDescription>Your reviewing statistics and performance metrics (mock data)</CardDescription></CardHeader><CardContent><div className="grid gap-4 md:grid-cols-3 lg:grid-cols-5">
                <div className="text-center p-4 bg-blue-50 rounded-lg"><div className="text-2xl font-bold text-blue-600">{profileData.totalReviews}</div><div className="text-sm text-gray-600">Total Reviews</div></div>
                <div className="text-center p-4 bg-green-50 rounded-lg"><div className="text-2xl font-bold text-green-600">{profileData.approvedReviews}</div><div className="text-sm text-gray-600">Approved</div></div>
                <div className="text-center p-4 bg-red-50 rounded-lg"><div className="text-2xl font-bold text-red-600">{profileData.rejectedReviews}</div><div className="text-sm text-gray-600">Rejected</div></div>
                <div className="text-center p-4 bg-purple-50 rounded-lg"><div className="text-2xl font-bold text-purple-600">{profileData.avgReviewTime}</div><div className="text-sm text-gray-600">Avg Review Time</div></div>
                <div className="text-center p-4 bg-amber-50 rounded-lg"><div className="text-2xl font-bold text-amber-600">{profileData.accuracy}%</div><div className="text-sm text-gray-600">Accuracy Rate</div></div>
            </div></CardContent></Card>
            <Card className="md:col-span-3"><CardHeader><CardTitle>Recent Activity</CardTitle><CardDescription>Your latest review activities (mock data)</CardDescription></CardHeader><CardContent><div className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg"><div className="flex items-center"><CheckCircle className="h-5 w-5 text-green-600 mr-3" /><div><p className="font-medium">Approved "Holiday Special" ad</p><p className="text-sm text-gray-500">2 hours ago</p></div></div><Badge variant="outline" className="text-green-600 border-green-600">Approved</Badge></div>
